 ...
 *
 * @param {number} n The number to convert
 * @returns {string} 4-digit hex string
 */
function hex4(n) {
 ...
 * @returns {string} Calculated 42-character hex command code
 */
function generateCode(unitId, channel, cmdCode, increment) {
    ...
    if (channel === 0) {
        code = `${
            CODE_PREFIX + hex4(increment) + CODE_SUFFIX + cleanUnitId + CODE_21
        }00${hex2(channel)}00${hex2(cmdCode)}`;
    } else {
        code = `${
            CODE_PREFIX + hex4(increment) + CODE_SUFFIX + cleanUnitId + CODE_21 + CODE_REMOTE + hex2(channel)
        }00${hex2(cmdCode)}`;
    }
...
function finalizeCode(code) {
    return Buffer.concat([Buffer.from([0x02]), Buffer.from(code, 'ascii'), Buffer.from([0x03])]);
}
